...
inline void copyVectorToUMat(const std::vector<_Tp>& v, cv::OutputArray um)
{
  if (v.empty())
    um.release();
  else
    cv::Mat(1, (int)(v.size() * sizeof(v[0])), CV_8U, (void*)&v[0]).copyTo(um);
}
/**
 * Function that computes the Harris responses in a
 * blockSize x blockSize patch at given points in the image
 */
static void HarrisResponses(const cv::Mat& img, const std::vector<cv::Rect>& layerinfo,
...
static void computeOrbDescriptors(const cv::Mat& imagePyramid,
...
#define GET_VALUE(idx)                                                                             \
  (x = pattern[idx].x * a - pattern[idx].y * b, y = pattern[idx].x * b + pattern[idx].y * a,       \
   ...
   cvRound(center[iy * step + ix] * (1 - x) * (1 - y) +                                            \
           center[(iy + 1) * step + ix] * (1 - x) * y + center[iy * step + ix + 1] * x * (1 - y) + \
 ...
 */
static void computeKeyPoints(const cv::Mat& imagePyramid, const cv::UMat& uimagePyramid,
                             const cv::Mat& maskPyramid, const std::vector<cv::Rect>& layerInfo,
                             const cv::UMat& ulayerInfo, const std::vector<float>& layerScale,
                             std::vector<cv::KeyPoint>& allKeypoints, int nfeatures,
                             ...
                             ORB::ScoreType scoreType, bool useOCL, int fastThreshold)
{
  ...
  for (level = 0; level < nlevels; level++)
  {
    ...
    cv::KeyPointsFilter::retainBest(
        keypoints, scoreType == ORB_Impl::HARRIS_SCORE ? 2 * featuresNum : featuresNum);
  ...
  if (scoreType == ORB_Impl::HARRIS_SCORE)
  {
    ...
    if (useOCL)
    {
      ...
      useOCL = ocl_HarrisResponses(uimagePyramid, ulayerInfo, ukeypoints, uresponses, nkeypoints, 7,
                                   HARRIS_K);
      if (useOCL)
      {
        CV_IMPL_ADD(CV_IMPL_OCL);
        uresponses.copyTo(responses);
        for (i = 0; i < nkeypoints; i++) allKeypoints[i].response = responses.at<float>(i);
      }
    }
    ...
    for (level = 0; level < nlevels; level++)
    {
      ...
      std::copy(allKeypoints.begin() + offset, allKeypoints.begin() + offset + nkeypoints,
                keypoints.begin());
      offset += nkeypoints;
  ...
  if (useOCL)
  {
    ...
    uploadORBKeypoints(allKeypoints, ukeypoints_buf, ukeypoints);
    useOCL = ocl_ICAngles(uimagePyramid, ulayerInfo, ukeypoints, uresponses, uumax, nkeypoints,
                          halfPatchSize);
    ...
    if (useOCL)
    {
      CV_IMPL_ADD(CV_IMPL_OCL);
      uresponses.copyTo(responses);
      for (i = 0; i < nkeypoints; i++) allKeypoints[i].angle = responses.at<float>(i);
    }
  }
 ...
 */
void ORB_Impl::detectAndCompute(cv::InputArray _image, cv::InputArray _mask,
                                ...
                                bool useProvidedKeypoints)
{
...
#ifdef HAVE_OPENCL
  bool useOCL =
      ocl::isOpenCLActivated() && OCL_FORCE_CHECK(_image.isUMat() || _descriptors.isUMat());
#else
  bool useOCL = false;
#endif

  cv::Mat image = _image.getMat(), mask = _mask.getMat();
  if (image.type() != CV_8UC1) cvtColor(_image, image, cv::COLOR_BGR2GRAY);
  ...
  if (!do_keypoints)
  {
    ...
    // and the algorithm used (i.e. BRIEF, running on 31x31 patches) we should compute the
    // approximate scale-factor that we need to apply. Then we should cluster all the computed
    // scale-factors and for each cluster compute the corresponding image.
    //
  ...
  cv::Size bufSize((int)cv::alignSize(level0_width + border * 2, 16),
  ...
  for (level = 0; level < nLevels; level++)
  {
    ...
    cv::Size sz(cvRound(image.cols * inv_scale), cvRound(image.rows * inv_scale));
  ...
  for (level = 0; level < nLevels; ++level)
  {
    ...
    cv::Size wholeSize(sz.width + border * 2, sz.height + border * 2);
    cv::Rect wholeLinfo =
        cv::Rect(linfo.x - border, linfo.y - border, wholeSize.width, wholeSize.height);
    ...
    if (level != firstLevel)
    {
      ...
      if (!mask.empty())
      {
        resize(prevMask, currMask, sz, 0, 0, cv::INTER_LINEAR_EXACT);
        if (level > firstLevel) threshold(currMask, currMask, 254, 0, cv::THRESH_TOZERO);
      }
  ...
  if (do_keypoints)
  {
    ...
    // the descriptor
    computeKeyPoints(imagePyramid, uimagePyramid, maskPyramid, layerInfo, ulayerInfo, layerScale,
                     keypoints, nfeatures, scaleFactor, edgeThreshold, patchSize, scoreType, useOCL,
                     fastThreshold);
  }
  else
  {
    cv::KeyPointsFilter::runByImageBorder(keypoints, image.size(), edgeThreshold);
  ...
  if (do_descriptors)
  {
    ...
    for (level = 0; level < nLevels; level++)
    {
      ...
      // boxFilter(working_mat, working_mat, working_mat.depth(), Size(5,5), Point(-1,-1), true,
      // BORDER_REFLECT_101);
    ...
    if (useOCL)
    {
      ...
      useOCL = ocl_computeOrbDescriptors(uimagePyramid, ulayerInfo, ukeypoints, udescriptors,
                                         upattern, nkeypoints, dsize, wta_k);
      if (useOCL)
      {
        CV_IMPL_ADD(CV_IMPL_OCL);
      }
...
#endif
    {
      ...
      computeOrbDescriptors(imagePyramid, layerInfo, layerScale, keypoints, descriptors, pattern,
                            dsize, wta_k);
    }
  }
}
...
cv::Ptr<ORB> ORB::create(int nfeatures, float scaleFactor, int nlevels, int edgeThreshold,
                         int firstLevel, int wta_k, ORB::ScoreType scoreType, int patchSize,
                         int fastThreshold)
{
  CV_Assert(firstLevel >= 0);
  return cv::makePtr<ORB_Impl>(nfeatures, scaleFactor, nlevels, edgeThreshold, firstLevel,
