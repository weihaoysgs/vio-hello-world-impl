...
template <>
bool ReprojectionErrorSE3XYZ<7>::Evaluate(const double *const *parameters,
                                          double *residuals,
  ...
  J_cam << f_by_z, 0, -f_by_zz * p[0], 0, f_by_z, -f_by_zz * p[1];

  if (jacobians != NULL) {
    if (jacobians[0] != NULL) {
      Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor> > J_se3(
          jacobians[0]);
      J_se3.setZero();
    ...
    if (jacobians[1] != NULL) {
      Eigen::Map<Eigen::Matrix<double, 2, 3, Eigen::RowMajor> > J_point(
          jacobians[1]);
...
template <>
bool ReprojectionErrorSE3XYZ<6>::Evaluate(const double *const *parameters,
                                          double *residuals,
                                          double **jacobians) const {
  Eigen::Quaterniond quaterd =
      toQuaterniond(Eigen::Map<const Vector3d>(parameters[0]));
...
template <>
bool PoseSE3Parameterization<7>::Plus(const double *x, const double *delta,
...
template <>
bool PoseSE3Parameterization<7>::ComputeJacobian(const double *x,
...
template <>
bool PoseSE3Parameterization<6>::Plus(const double *x, const double *delta,
                                      double *x_plus_delta) const {
  Eigen::Map<const Eigen::Vector3d> trans(x + 3);
  SE3 se3_delta = SE3::exp(Eigen::Map<const Vector6d>(delta));

  Quaterniond quaterd_plus =
      se3_delta.rotation() * toQuaterniond(Eigen::Map<const Vector3d>(x));
...
template <>
bool PoseSE3Parameterization<6>::ComputeJacobian(const double *x,
                                                 double *jacobian) const {
...
template <>
void PosePointParametersBlock<7>::getPose(int idx, Quaterniond &q,
                                          Vector3d &trans) {
...
template <>
void PosePointParametersBlock<7>::setPose(int idx, const Quaterniond &q,
...
template <>
void PosePointParametersBlock<6>::getPose(int idx, Quaterniond &q,
                                          Vector3d &trans) {
...
template <>
void PosePointParametersBlock<6>::setPose(int idx, const Quaterniond &q,
                                          const Vector3d &trans) {
  double *pose_ptr = values + idx * 6;
