...
class ReprojectionErrorSE3XYZ
    : public tceres::SizedCostFunction<2, PoseBlockSize, 3>
{
 public:
  ReprojectionErrorSE3XYZ(double f_, double cx_, double cy_,
                          double observation_x, double observation_y)
      : f(f_),
        cx(cx_),
        cy(cy_),
...
/// 6 (rotation vector + translation vector)
template <int PoseBlockSize>
class PoseSE3Parameterization : public tceres::LocalParameterization
{
 public:
...
class PosePointParametersBlock
{
 public:
  ...
  PosePointParametersBlock(int pose_num, int point_num)
      : poseNum(pose_num), pointNum(point_num) {
    values = new double[pose_num * PoseBlockSize + point_num * 3];
  }
